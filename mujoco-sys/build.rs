...
fn main() {
    ...
    {
        Ok(_) => ("lib", if env::var("CARGO_CFG_TARGET_VENDOR").unwrap() == "apple" { "dylib" } else { "so" }, "/usr/local"),
    ...
    if option_env!("DOCS_RS").is_none() {
        ...
        println!("cargo:rerun-if-changed={}", std::fs::read_link(mj_lib.join(&lib_file).to_str().unwrap()).expect(format!("Expected symbolic link to {}", &lib_file).as_str()).to_str().unwrap());
    }
}
